...
namespace SI
{
	...
	void Query::batquery_AVL(SIStringList& slist, linklist<IndexListNode>& res)
	{
		...
		{
			res.push_back(IndexListNode(&pages[id], cnt[id]));
		}
	}
	void Query::batquery_HASH(SIStringList& slist, linklist<IndexListNode>& res)
	{
		memset(cnt, 0, sizeof(cnt));
		memset(T, 0, sizeof(T));
		memset(sigmac2, 0, sizeof(sigmac2));
